...
public class SAGR {
    ...
    static void TT(int DataBase,double findBestRouteTime, double findNegativeReducedCostRouteTime, double forcedCoveredPairInitialPatternTimeLim, double OneBestPatternTimeLim, double NegativeReducedCostPatternTimeLim, double TotalTimeLim) throws IOException, InputDataReader.InputDataReaderException, IloException {
        /*定义各数据集相关参数*/
        int ActFlightNum = 0;
        int MtsNum = 0;
        int AircNum = 0;
        ...
        System.out.println("findBestRouteTime = " + findBestRouteTime);
        System.out.println("findNegativeReducedCostRouteTime = " + findNegativeReducedCostRouteTime);
        ...
        /*航班数据导入*/
        InputDataReader FlightReader = new InputDataReader("D:\\IDEA\\works\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\Flight.txt");
        ...
        /*飞机数据导入*/
        InputDataReader AircReader = new InputDataReader("D:\\IDEA\\works\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\Aircraft.txt");
        ...
        /*时隙数据导入*/
        DataReader ApSlotDepReader = new DataReader("D:\\IDEA\\works\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\ApSlotDep.txt");
        DataReader ApSlotArrReader = new DataReader("D:\\IDEA\\works\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\ApSlotArr.txt");
...
//        if(seqFlag == 0){
//            InitialRouteNumRString = "D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\RouteNum"+InitialRouteSize+".txt";
//            InitialRouteRString = "D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\Route"+InitialRouteSize+".txt";
//        }else{
//            InitialRouteNumRString = "D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\InitialRouteNum"+InitialRouteSize+".txt";
//            InitialRouteRString = "D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\InitialRoute"+InitialRouteSize+".txt";
//        }

//        InitialRouteNumRString = "D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\InitialRouteNum"+InitialRouteSize+".txt";
//        InitialRouteRString = "D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\InitialRoute"+InitialRouteSize+".txt";
        InitialRouteNumRString = "D:\\IDEA\\works\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\RouteNum"+InitialRouteSize+".txt";
        InitialRouteRString = "D:\\IDEA\\works\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\Route"+InitialRouteSize+".txt";
        ...
        /*初始机位Pattern导入*/
        DataReader InitialPatternNumR = new DataReader("D:\\IDEA\\works\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\InitialPatternNum"+InitialPatternNum+"~"+gateTypeNum+".txt");
        DataReader InitialPatternR = new DataReader("D:\\IDEA\\works\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\InitialPattern"+InitialPatternNum+"~"+gateTypeNum+".txt");
        for(int ap=0; ap< ApNum; ap++) {
        ...
        if(seqFlag == 1){
            ...
            for(int airc=0; airc<AircNum; airc++){
                y[airc] = Math.toIntExact(Math.round(ySARqSARIPobj[airc]));
            }
...
//                    System.out.print(Arrays.toString(conOpt[airc][i]));
//                    System.out.println(Arrays.toString(yCoverCfPair[conflag]));
                    conflag += 1;
...
//                System.out.println(Arrays.toString(AirpTypeLegnumPattern[ap][tp][AirpTypePatternNum[ap][tp]]));
            ...
            while(Arrays.stream(AircReducedCostFlag).sum()<AircNum && SAloop<SAloopLim){
                ...
                for(int airc=0; airc<AircNum; airc++){
                    ...
                    double[] ReducedcostLegnumDlytimeSwapnumRoute =
                            SAR.OneBestRouteGRphase1(findBestRouteTime, airc, LpDualValue, AllGRLpOptDualValue, AllyCoverCf, AllyCoverCfPair,
                    ...
                    int[][] AB;
                    AB = SAR.NegativeReducedCostRouteGRphase1(findNegativeReducedCostRouteTime, airc, LpDualValue, AllGRLpOptDualValue, AllyCoverCf, AllyCoverCfPair,
                            AllSAROptSolFea2GR, AllSAROptSolInfea2GR, AllGRLpPhase1InfeaDualValue,
                            AllGRIPOptVal, SAL,
                            CopyFlightOFID, CopyFlightDepS, CopyFlightArrS,
                            CopyFlightDepT, CopyFlightArrT, CopyFlightDepTSlot, CopyFlightArrTSlot,
...
//            FileWriter RouteNum510FW = new FileWriter("D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\betterRouteNum"+1000+".txt");
...
//            FileWriter Route510FW = new FileWriter("D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\betterRoute"+1000+".txt");
//            for(int airc=0; airc< Aircraft.length; airc++){
//                for (int i = 0; i < AircRouteNum[airc]; i++) {
...
//            Route510FW.close();
            ...
            for(int airc=0; airc<AircNum; airc++){
                y[airc] = (int) Math.round(ySARqSARIPobj[airc]);
            }
            System.out.println("Airc choose route's index = "+Arrays.toString(y));
...
//                System.out.println(Arrays.toString(chooseRoute));
            ...
            while (true){
                ...
                //定理1判定 第一项赋值
                double[][] Th1FirstTerm = new double[ApNum][2];
                for(int cf=0; cf<yCoverCf01.length; cf++){
                    if(yCoverCf01[cf]==1){
                        int cfDepS = CopyFlightDepS[cf];
                        int cfArrS = CopyFlightArrS[cf];
                        int cf2GateType = (CopyFlightType[cf] < GateTypeDivideline ? 0 : 1);
                        Th1FirstTerm[cfDepS][cf2GateType] += phase1DualValue[0][cf];
                        Th1FirstTerm[cfArrS][cf2GateType] += phase1DualValue[1][cf];
                    }
                }
                for(int p=0; p<yCoverCfPair.length; p++) {
                    int[] Pair = yCoverCfPair[p];
                    int ap = CopyFlightArrS[Pair[0]];
                    int tp = (CopyFlightType[Pair[0]] < GateTypeDivideline ? 0 : 1);
                    Th1FirstTerm[ap][tp] += phase1DualValue[2][p];
                }
                ...
                for(int ap=0; ap<ApNum; ap++) {
                    ...
                    for(int tp = 0; tp < 2; tp++) {
                        ...
                        //定理1 第二项 + 判别
                        double deltaHat = phase1DualValue[3][ap*2+tp]+ Math.max(0, -ReducedcostLegnumPattern[0]);
                        if( Th1FirstTerm[ap][tp]-GateCap*deltaHat>0 ){
                            Th1BreakFlag = 1;
                            break;
                        }
                ...
                //定理1条件成立 加 NGcut
                if(Th1BreakFlag==1||GRphase1loop>=10){
...
//        System.out.println("------------\n\n\n");
        ...
        for(int i=0; i<=5; i++){
            for(int j=1; j<=1; j++){
                for(int k=2; k<=2; k++){
                    int DataBase = 10;
                    double findBestRouteTime =  0.05;
                    double findNegativeReducedCostRouteTime = 0.05;
                    double forcedCoveredPairInitialPatternTimeLim = 4.2+0.02*i;
                    double OneBestPatternTimeLim = 0.05;
                    double NegativeReducedCostPatternTimeLim = 0.05;
                    double TotalTimeLim = 1800;

                    OutConsole.write("D:\\IDEA\\works\\src\\SACG_15minTiaoTime10\\10\\database10 1700 "+findBestRouteTime+" "+findNegativeReducedCostRouteTime+" "+forcedCoveredPairInitialPatternTimeLim+" "+OneBestPatternTimeLim+" "+NegativeReducedCostPatternTimeLim+".txt");
                    System.out.println("------------");
                    TT(DataBase, findBestRouteTime, findNegativeReducedCostRouteTime,forcedCoveredPairInitialPatternTimeLim, OneBestPatternTimeLim, NegativeReducedCostPatternTimeLim, TotalTimeLim);
                    System.out.println("------------\n\n\n");
                }
            }
        }
