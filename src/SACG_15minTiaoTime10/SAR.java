...
//        System.out.println(" ");
        ...
        for(int airc=0; airc<AircNum; airc++){
            for(int j=0; j<y[airc].length; j++){
                if(Math.round(cplex.getValue(y[airc][j]))>0){
                    AircChoose[airc] = j;
                }
            }
        }
...
//            System.out.println("\n"+cp.getStatus());
    ...
    static double[] OneBestRouteGRphase1(double findBestRouteTime, int AircID,  double[][] LpDualValue,
...
//        cp.addLe(obj, -epson);//加
        ...
        cp.setParameter(IloCP.IntParam.Workers, 1);//加速
//        cp.setParameter(IloCP.IntParam.SearchType, IloCP.ParameterValues.MultiPoint);
        cp.setParameter(IloCP.DoubleParam.TimeLimit,findBestRouteTime);
...
//                System.out.println("AircID: "+AircID+" find " +findflag+ " route(s)");
    ...
    static int[][] NegativeReducedCostRouteGRphase1(double findNegativeReducedCostRouteTime,int AircID,  double[][] LpDualValue,
                                                    double[][][] AllGRLpOptDualValue, int[][] AllyCoverCf, int[][][] AllyCoverCfPair,
                                                    int[] AllSAROptSolFea2GR, int[] AllSAROptSolInfea2GR, double[][][] AllGRLpPhase1InfeaDualValue,
                                                    double[] AllGRIPOptVal,double L,
                                                    int[] CopyFlightOFID, int[] CopyFlightDepS, int[] CopyFlightArrS,
...
//        cp.addEq(cp.element(CopyFlightArrS, flightvars[MaxLegNum-1]), AircEndS);
        ...
        cp.setParameter(IloCP.IntParam.Workers, 12);//加速
        cp.setParameter(IloCP.IntParam.SearchType, IloCP.ParameterValues.MultiPoint);
        cp.setParameter(IloCP.DoubleParam.TimeLimit,findNegativeReducedCostRouteTime);
...
//        for(int f=0; f<FlightNum; f++){
//            if(Math.round(cplex.getValue(z[f]))>0){
//                System.out.println(cplex.getValue(z[f]));
...
//        System.out.println("DataBase = "+DataBase+", Initial1RouteNum = "+Initial1RouteNum+", NegativeReducedCostRouteNum = "+NegativeReducedCostRouteNum);
        ...
        InputDataReader FlightReader = new InputDataReader("D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\Flight.txt");
        ...
        InputDataReader AircReader = new InputDataReader("D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\Aircraft.txt");
        int[][] Aircraft = AircReader.readIntArrayArray();

        DataReader ApSlotDepReader = new DataReader("D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\ApSlotDep.txt");
        DataReader ApSlotArrReader = new DataReader("D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\ApSlotArr.txt");
...
//        }
        ...
        for(int a=1; a<=1; a++){
            for(int b=1;b<=1; b++){
                ...
                int[] AircRouteNum = new int[AircNum];
                int[][][] AircLegnumDlytimeSwapnumRoute = new int[AircNum][][];
                DataReader InitialRouteNumR = new DataReader("D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\InitialRouteNum"+Initial1RouteNum+".txt");
                DataReader InitialRouteR = new DataReader("D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\InitialRoute"+Initial1RouteNum+".txt");
                for (int airc = 0; airc < AircNum; airc++) {
                    AircRouteNum[airc] = InitialRouteNumR.next();
                    AircLegnumDlytimeSwapnumRoute[airc] = new int[AircRouteNum[airc]][3+MaxRouteLegNum];
                }
...
//                FileWriter RouteNum510FW = new FileWriter("D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\RouteNum"+1000+".txt");
...
//                FileWriter Route510FW = new FileWriter("D:\\idea_javaproject\\idea_javaproject\\src\\SACG_15minTiaoTime10\\Data\\"+DataBase+"\\Route"+1000+".txt");
//                for(int airc=0; airc< Aircraft.length; airc++){
//                    for (int i = 0; i < AircRouteNum[airc]; i++) {
